...
{/if}
	...
	<font size="+1"><b>{$individual.full_name|escape}</b></font>
...
{if $father.person_id > 0}
	<b><a href="/family/{$father.person_id}">{$father.full_name|escape}</a></b><br /><br />
...
{if $mother.person_id > 0}
		<b><a href="/family/{$mother.person_id}">{$mother.full_name|escape}</a></b><br /><br />
...
{if $marriage.person_id > 0}
	<b><a href="/family/{$marriage.person_id}">{$marriage.full_name|escape}</a></b><br /><br />
...
{/if}
	...
	{foreach item=child from=$marriage.children}
		<tr><td>{if $child.protected!=1}{include file="person_nav.tpl" nav_id=$child.person_id direction="flat" return_to="/family/$person_id"}{/if}</td>
			...
			<td>{if $child.protected==1}{$child.full_name}{else}<a href="/family/{$child.person_id}">{$child.full_name|escape}</a>{/if}</td>
			<td>{$child.gender}</td>
